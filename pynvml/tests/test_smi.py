...
def test_memory_utilization(ngpus, smi):
    for i in range(ngpus):
        result = smi.DeviceQuery('utilization.memory')['gpu'][i]['utilization']['memory_util']
        assert result >= 0
...
def test_temperature(ngpus, smi):
    for i in range(ngpus):
        ...
        max_temp = smi.DeviceQuery('temperature.gpu')['gpu'][i]['temperature']['gpu_temp_max_threshold']
